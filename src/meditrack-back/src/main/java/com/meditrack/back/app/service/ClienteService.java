...
public class ClienteService {
    ...
    private void mapDataToCliente(Map<String, Object> body, Cliente cliente, String usuario) {
        ...
        cliente.setTipoEstablecimiento(TipoEstablecimiento.valueOf(body.get("tipoEstablecimiento").toString()));
        cliente.setUsuarioResponsable(usuario);

        if (body.get("latitud") != null) {
            cliente.setLatitud(new BigDecimal(body.get("latitud").toString()));
        }
        if (body.get("longitud") != null) {
            cliente.setLongitud(new BigDecimal(body.get("longitud").toString()));
        }
    }
}