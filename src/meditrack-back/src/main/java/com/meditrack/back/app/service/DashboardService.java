...
public class DashboardService {
    ...
    public DashboardKpiDTO obtenerMetricasDashboard(boolean historico) {
        ...
        double tasaIncidencias = totalEnvios > 0
                ? Math.round(((double) incidenciasCount / totalEnvios) * 100.0 * 10.0) / 10.0
                : 0.0;
        ...
        List<Map<String, Object>> top5Medicamentos = envios.stream()
                ...
                .collect(Collectors.groupingBy(d -> d.getMedicamento().getNombre(),
        ...
        Map<String, List<Envio>> enviosPorCliente = envios.stream().filter(e -> e.getDestinatario() != null)
        ...
        List<Map<String, Object>> top3Clientes = enviosPorCliente.entrySet().stream()
                ...
                .map(e -> {
                    ...
                    m.put("porcentaje",
                            totalClientesUnicos > 0 ? Math.round((1.0 / totalClientesUnicos) * 100.0 * 10.0) / 10.0
                                    : 0.0);
                    m.put("medicamentoTop",
                            e.getValue().stream().flatMap(env -> env.getDetalles().stream())
                                    .filter(d -> d.getMedicamento() != null && d.getCantidad() != null)
                                    .collect(Collectors.groupingBy(
                                            d -> d.getMedicamento().getNombre(),
                                            Collectors.summingLong(d -> d.getCantidad())))
                                    .entrySet().stream()
        ...
        return new DashboardKpiDTO(volumenEnvios,
                envios.stream().filter(e -> e.getEstado() == EstadoEnvio.ENTREGADO).count(), tasaIncidencias,
                tipoMasRetira, tipoMasRecibe, top5Medicamentos, top3Clientes);
    }
    ...
    private String obtenerTipoDominante(List<Envio> envios, java.util.function.Function<Envio, String> mapper,
            Map<String, String> mapaTipos) {
        return envios.stream().map(mapper).map(n -> mapaTipos.getOrDefault(n, "DESCONOCIDO"))
                .filter(t -> !t.equals("DESCONOCIDO")).collect(Collectors.groupingBy(t -> t, Collectors.counting()))
                .entrySet().stream().max(Map.Entry.comparingByValue()).map(Map.Entry::getKey).orElse("Ninguno");
    }
